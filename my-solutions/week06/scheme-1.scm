  ...
  (cond ((constant? exp) exp)
	...
	((if-exp? exp)
	 (if (eval-1 (cadr exp))
	     ...
	     (eval-1 (cadddr exp))))
	((lambda-exp? exp) exp)
	((let-exp? exp) (let-1 (cadr exp) (caddr exp)))
	((and-exp? exp) (and-1 (map-1 eval-1 (cdr exp))))
	((map-exp? exp) (map-1 (eval-1 (car exp)) (map-1 (cdr exp))))
...
(define (and-1 args)
	(cond ((null? args) #t)
				...
				(else (and (car args) (and-proc (cdr args))))))

(define (map-1 proc l)
	(if (null? l)
	 '()
		(cons (apply-1 proc (list (car l))) (map-1 proc (cdr l)))))

(define (let-1 args body)
	(apply-1 (list 'lambda (map car args) body) (map (lambda (a) (eval-1 (cadr a))) args)))

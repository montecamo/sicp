...
static void print_banner(void)
{
  if (STk_lookup_variable(PRINT_BANNER, NIL) != Ntruth){
    Fprintf(STk_curr_eport, "Welcome to the STk interpreter version 4.0.1-ucb%s [%s]\n", 
	    STK_VERSION, MACHINE);
    /* the copyright symbol does not work on WIN32 system (or under emacs) */
    Fprintf(STk_curr_eport, "Copyright (c) 1993-1999 Erick Gallesio - ");
    Fprintf(STk_curr_eport, "I3S - CNRS / ESSI <eg@unice.fr>\n");
    Fprintf(STk_curr_eport, "Modifications by UCB EECS Instructional Support Group\n");
    Fprintf(STk_curr_eport, "Questions, comments, or bug reports to <inst@EECS.Berkeley.EDU>.\n");
  }
}
...
{
  Fprintf(STk_curr_eport, "Warning: DISPLAY variable is not set. Graphics will NOT be initialized.\n");
}
#endif
...
static void load_user_init_file(void)
{
  ...
  /* Also try userinit.stk in the library dir -- for berkeley.scm loading */
  sprintf(file, "%s/STk/%s", STk_library_path, init_file2);
  if (STk_load_file(file, FALSE, STk_selected_module) == Truth) return;
}
...
static void init_library_path(char *argv0)
{
  ...
  if ((s = getenv("STK_LIBRARY"))) {
    ...
    strcpy(STk_library_path, s);
      ...
      char *stklibpath = STK_LIB_PATH;
      /* SCM canonical_argv0 = STk_resolve_link(argv0, 0); */
      SCM canonical_argv0 = STk_resolve_link(stklibpath, 0);
  ...
  for( ; ; ) {
    ...
    if (STk_dumped_core) {
	 ...
	 if (STk_internal_eval_string("(catch (repl-display-result *repl-result*))",
	   ...
	   Putc('\n', STk_curr_oport);
	   if (STk_transcript_port) {
		STk_print(x, STk_transcript_port, WRT_MODE);
	   	Putc('\n', STk_transcript_port);
		STk_flush(STk_transcript_port);
	   }
	 } 

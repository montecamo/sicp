...
int writable(char *dir)
{
	...
	/* We consider the directory writable if we can create a file in it. */
	sprintf(filename, "%s\\%s", dir, "delete.me");
	fh = _creat(filename, _S_IREAD | _S_IWRITE);
	rv = (!(fh < 0));
	if (rv) {
		_close(fh);
		_unlink(filename);
...
int try_home(char *dir)
{
	char envvar[517];

	if (! writable(dir)) {
		return 0; /* fail unless writable */
	}
	sprintf(envvar, "HOME=%s", dir);
	if (! _putenv(envvar)) {
		return 0; /* fail unless variable is set */
	}
	return 1;
}

void set_home(char *program_name)
{
	char home[512], *pos = NULL, *homedrive = NULL, *homepath = NULL;

	/* If we already have a HOME environment variable, then set_home()
	 * won't touch it.
	 */
	if (getenv("HOME") != NULL)
		return;

	/* First successful call to try_home() wins. */
	...
	/* 2.
	 * try program folder -- if we're C:\Program Files\STk\MS-Win32\STk.exe,
	 * chop off last two dir. elements and use that directory.
	 */
	strcpy(home, program_name);
	if ((pos = strrchr(home, '\\')) != NULL) {
		pos[0] = '\0'; /* chop off program name */
		if ((pos = strrchr(home, '\\')) != NULL) {
			pos[0] = '\0'; /* chop off last directory */
			if (try_home(home)) /* and use it */
				return;
		}
	}

...
SCM STk_transcript_port, STk_transcript_stdout, STk_transcript_stderr;
...
void STk_init_standard_ports(void)
{
  ...
  STk_transcript_port = NULL;
  STk_transcript_stdout = STk_stdout;
  STk_gc_protect(&STk_transcript_stdout);
  STk_transcript_stderr = STk_stderr;
  STk_gc_protect(&STk_transcript_stderr);
}
...
PRIMITIVE STk_change_standard_ports(SCM in, SCM out, SCM err)
{
  ...
  STk_curr_oport = out;
  STk_curr_eport = err;
  STk_transcript_stdout = out;
  STk_transcript_stderr = err;
...
PRIMITIVE STk_transcript_on(SCM filename)
{
	STk_transcript_port = makeport(CHARS(filename), tc_oport, "w", TRUE);
    STk_gc_protect(&STk_transcript_port);
	return UNDEFINED;
}

PRIMITIVE STk_transcript_off(void)
{
	if (STk_transcript_port) {
		STk_flush(STk_transcript_port);
		STk_close_port(STk_transcript_port);
		STk_transcript_port = NULL;
	}
	return UNDEFINED;
}

...
{
#if defined(WIN32)
  Err("posix-symlink: not implemented", NIL);
  return Ntruth;
#else
  int r;
...
#else
  Err("posix-fork: not implemented in Windows", NIL);
  return Ntruth;
#endif
}
...
#else
  Err("posix-wait: not implemented in Windows", NIL);
  return Ntruth;
#endif
}
...
int uname(struct utsname *u)
{
	...
	if (version < 0x80000000) {
		...
		build = (DWORD)(HIWORD(version) & ~0x8000);
	} else {
		build = 0; 
	}
	strcpy(u->sysname, sys);
	sprintf(u->release, "%d.%d", major, minor);
	if (build) {
		sprintf(u->version, "%d", build);
		strcat(u->release, ".");
		strcat(u->release, u->version);
	} else {
...
static PRIMITIVE posix_domain_name(void)
{
  ...
  return STk_makestring(name);
#else
  Err("posix-domain-name: not implemented in Windows", NIL);
  return Ntruth;
#endif
}

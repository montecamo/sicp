;;;;
;;;; $Id: pexpr.scm.in,v 1.2 2006/04/24 20:06:12 mars Exp $
;;;;
...
;;;;
;;;; $Log: pexpr.scm.in,v $
;;;; Revision 1.2  2006/04/24 20:06:12  mars
;;;;
...
(define psd-error (with-module scheme error))
...
(define psd-car
  (let ((car car))
    (lambda (x)
	  ...
	  (psd-error "psd-car: argument not a psd-pair ~a~%" x)))))
...
(define psd-cdr
  (let ((cdr cdr))
    (lambda (x)
	  ...
	  (psd-error "psd-cdr: argument not a psd-pair ~a~%" x)))))
...
(define psd-cddar (lambda (x) (psd-cdr (psd-cdr (psd-car x)))))
